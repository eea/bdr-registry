...
class EmailTemplates(views.StaffuserRequiredMixin, generic.TemplateView):
...
class EmailTemplatesFilter(views.StaffuserRequiredMixin, base.FilterView):

    raise_exception = True

    def process_name(self, object, val):
        url = reverse("management:email_template_view", kwargs={"pk": object.pk})
    ...
    def get_queryset(self, opt):
        ...
        if "search" in opt and opt["search"]:
            search_filters = Q(name__icontains=opt["search"]) | Q(
                subject__icontains=opt["search"]
            )
...
class EmailTemplateEdit(
    ...
    def get_context_data(self, **kwargs):
        back_url = reverse(
            "management:email_template_view", kwargs={"pk": self.object.pk}
        )
        breadcrumbs = [
            Breadcrumb(reverse("home"), title=_("Registry")),
            Breadcrumb(reverse("management:email_templates"), _("Email templates")),
