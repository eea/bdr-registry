...
def import_companies(csv_file):
    for byte_row in csv.DictReader(csv_file):
        ...
        org_data = {
            "account": account,
            "date_registered": timezone.make_aware(raw_date - HOURS_2, timezone.utc),
        ...
        models.Person.objects.create(company=org, **p1_data)
        if any(p2_data.values()):
            models.Person.objects.create(company=org, **p2_data)
...
def update_companies_from_ldap():
    ...
    res = conn.search_s(
        "ou=Business Reporters,o=Eionet,l=Europe",
        ldap.SCOPE_SUBTREE,
    ...
    for dn, attrs in res:
        [uid] = attrs["uid"]
        [cn] = attrs["cn"]
        org_name, country_name = [v.strip().decode("utf-8") for v in cn.rsplit("/", 1)]
        ...
        log.info("Company: uid=%r pk=%r name=%r", org.account.uid, org.pk, org.name)
...
def update_companies_from_csv(csv_file, commit=False):
    ...
    for byte_row in csv.DictReader(csv_file, dialect="excel-tab"):
        ...
        old = [org.addr_street, org.addr_place1, org.addr_place2, org.addr_postalcode]
        new = [
            row("address", "Address", "Street"),
        ...
        for n in range(1, 3):
            person_data = {
                "first_name": row(
                    "CP%d - first name" % n, "first name C%d" % n, "Given_%d" % n
                ),
                "family_name": row(
                    "CP%d - Last name" % n, "last name C%d" % n, "Surname_%d" % n
                ),
                "email": row("CP%d - e-mail" % n, "Email C%d" % n, "Email_%d" % n),
                "phone": row(
                    "CP%d - Tel" % n,
            ...
            log.info("uid=%s: new person email=%s", account.uid, person_data["email"])
