...
def sync_accounts_with_ldap(accounts):
    ...
    for account in accounts:
        for company in account.companies.all():
    ...
    if request.POST.get("perform_create"):
        ...
        if request.POST.get("email"):
            return send_password_email(
                modeladmin, request, organisations_without_account
            )
    ...
    for company in organisations:
        for person in company.people.all():
            if company.obligation.code == "ods":
                ...
                raise RuntimeError("Unknown obligation %r" % company.obligation.code)

            mail_to = [person.email]
            message = mail.EmailMessage(subject, html, mail_from, mail_to, mail_bcc)
...
def send_password_email(modeladmin, request, queryset):
    ...
    if request.POST.get("perform_send"):
        ...
        messages.add_message(
            request, messages.INFO, "Emails have been sent to %d people." % n
        )
        return

    return TemplateResponse(
        request,
        "company_email_password.html",
        {
            "organisations_with_account": organisations_with_account,
            "queryset": queryset,
            "action_checkbox_name": helpers.ACTION_CHECKBOX_NAME,
...
def create_reporting_folder(modeladmin, request, queryset):
    if not (settings.BDR_API_URL and settings.BDR_API_AUTH):
        messages.add_message(
            request, messages.ERROR, "BDR_API_URL and BDR_API_AUTH not configured"
        )
        return
...
class CustomBooleanFieldListFilter(admin.FieldListFilter):
    def __init__(self, field, request, params, model, model_admin, field_path):
        ...
        super(CustomBooleanFieldListFilter, self).__init__(
...
admin.FieldListFilter.register(
    lambda f: isinstance(f, (models.OneToOneField)), CustomBooleanFieldListFilter
)
...
class OrganisationAdmin(ReadOnlyAdmin):
    ...
    search_fields = ["name", "account__uid", "addr_postalcode", "vat_number", "eori"]
...
class PersonAdmin(ReadOnlyAdmin):
    ...
    def export(self, request):
        ...
        out = csv.writer(of)
        out.writerow(
            ["userid", "companyname", "country", "contactname", "contactemail"]
        )
...
class UserAdminForm(UserChangeForm):
    obligations = forms.ModelMultipleChoiceField(
        ...
        required=False,
        widget=FilteredSelectMultiple(verbose_name="Obligations", is_stacked=False),
    )
