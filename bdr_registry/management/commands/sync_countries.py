...
class Command(BaseCommand):
    ...
    help = (
        "Gets a country list from %s and adds to the application's DB the "
    ...
    def handle(self, *args, **options):
        ...
        if r.status_code != 200:
            raise CommandError("GET {0} returned {1}".format(url, r.status_code))
        ...
        for country_dict in countries:
            try:
                country, created = Country.objects.get_or_create(
                    name=country_dict["name"], code=country_dict["iso"].lower()
                )
                if created:
