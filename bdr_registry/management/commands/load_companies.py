...
class Command(BaseCommand):
    help = (
        "Creates companies from a request provided.updated_since looks like DD/MM/YYYY"
    )
    ...
    def handle(self, *args, **options):
        ...
        for company in companies:
            country = Country.objects.filter(
                code=company["address"]["country"]["code"].lower()
            ).first()
            obligation = Obligation.objects.filter(
                code=company["domain"].lower()
            ).first()
            ...
            self.stdout.write("Company {name} created.".format(name=company["name"]))
